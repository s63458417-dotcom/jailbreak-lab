...
interface StoreContextType {
  ...
  allChats: Record<string, ChatSession>; // Exposed for analytics
}

const StoreContext = createContext<StoreContextType | undefined>(undefined);

// Helper to safely parse JSON without crashing the app
...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  // Load Personas with Safety Check
  const [personas, setPersonas] = useState<Persona[]>(() => {
    return safeJSONParse('pentest_personas', INITIAL_PERSONAS);
  });

  // Load Chat Sessions with Safety Check
