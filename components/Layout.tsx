    ...
    <div className="h-screen flex bg-neutral-950 text-neutral-200 font-sans overflow-hidden">
      ...
      `}>
            ...
            {personas && personas.length > 0 ? personas.map(p => (
                ...
                >
                    <div className="relative flex items-center justify-center w-5 h-5">
                      <span className={`w-2 h-2 rounded-full absolute ${p.isLocked ? 'bg-neutral-600' : 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]'}`}></span>
                    </div>
                    <span className="truncate flex-1 font-medium text-left">{p.name}</span>
                    {p.isLocked && <svg className="w-3.5 h-3.5 text-neutral-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>}
                </button>
            )) : (
              <div className="px-3 py-2 text-xs text-neutral-600 italic">No personas deployed.</div>
            )}
        </nav>
        ...
        <div className="p-4 border-t border-neutral-800 bg-neutral-900/50 flex-shrink-0">
            ...
            <Button variant="secondary" fullWidth onClick={logout} className="text-xs h-8 py-0 border-neutral-700 hover:border-neutral-500">
               Terminate Session
            </Button>
        </div>
      </aside>
